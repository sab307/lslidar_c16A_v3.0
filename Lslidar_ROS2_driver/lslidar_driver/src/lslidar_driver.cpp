...
namespace lslidar_driver {
    ...
    bool LslidarDriver::loadParameters() {
        ...
        this->declare_parameter<std::string>("lidar_type_override", "");
        ...
        this->get_parameter("lidar_type_override", lidar_type_override_);
        difop_shared_port_ = (difop_udp_port == msop_udp_port);
    ...
    bool LslidarDriver::createRosIO() {
        ...
        if (difop_shared_port_)
            LS_INFO << "DIFOP shares the MSOP port " << msop_udp_port
                    << "; demultiplexing by packet header" << LS_END;
    ...
    void LslidarDriver::difopPoll() {
        ...
        while (rclcpp::ok()) {
            // keep reading
            int rc = getDifopPacket(difop_packet_ptr);
            ...
            if (rc == 0) {
                ...
                if(fpga_type == 0) {
                    ...
                    if (!lidar_type_override_.empty()) {
                        applyLidarTypeOverride();
                    } else {
                        determineLidarType();
                    }
                } 
    ...
    bool LslidarDriver::poll() {
        ...
        // Some units are configured to send DIFOP to the MSOP port. When
        // difop_port == msop_port, split the two streams by header here and hand
        // the DIFOP frames to difopPoll() through difop_queue_.
        if (difop_shared_port_ && packet->data[0] == 0xa5 && packet->data[1] == 0xff &&
            packet->data[2] == 0x00 && packet->data[3] == 0x5a) {
            {
                std::lock_guard<std::mutex> lock(difop_queue_mutex_);
                if (difop_queue_.size() < 8) difop_queue_.push(*packet);
            }
            difop_queue_cv_.notify_one();
            return true;
        }
        msop_return_byte_ = packet->data[1204];
    ...
    int LslidarDriver::getDifopPacket(lslidar_msgs::msg::LslidarPacket::UniquePtr &pkt) {
        if (!difop_shared_port_) return difop_input_->getPacket(pkt);

        std::unique_lock<std::mutex> lock(difop_queue_mutex_);
        if (!difop_queue_cv_.wait_for(lock, std::chrono::milliseconds(1000),
                                      [this] { return !difop_queue_.empty(); })) {
            return 1;   // same meaning as InputSocket: poll() timeout, keep looping
        }
        *pkt = difop_queue_.front();
        difop_queue_.pop();
        return 0;
    }

    bool LslidarDriver::applyLidarTypeOverride() {
        const std::string &type = lidar_type_override_;
        ...
        if (type == "c16_3") {
            ...
            distance_unit = 0.25;
            lidar_type = "c16_3";
            LS_INFO << "lidar type (override): C16, version 3.0" << LS_END;
        } else if (type == "C16") {
            for (int k = 0; k < 16; ++k) {
                sin_scan_altitude[k] = sin(c16_vertical_angle[k] * DEG_TO_RAD);
                cos_scan_altitude[k] = cos(c16_vertical_angle[k] * DEG_TO_RAD);
            }
            ring_ = 16;
            lidar_number_ = 16;
            R1 = R1_;
            ...
            lidar_type = "C16";
            LS_INFO << "lidar type (override): C16" << LS_END;
        } else if (type == "c32_3") {
            for (int k = 0; k < 32; ++k) {
            ...
            lidar_type = "c32_3";
            LS_INFO << "lidar type (override): C32, version 3.0" << LS_END;
        } else if (type == "C32") {
            for (int k = 0; k < 32; ++k) {
                sin_scan_altitude[k] = sin(c32_vertical_angle[k] * DEG_TO_RAD);
                cos_scan_altitude[k] = cos(c32_vertical_angle[k] * DEG_TO_RAD);
            }
            ring_ = 32;
            lidar_number_ = 32;
            R1 = R1_;
            conversionAngle = conversionAngle_;
            lidar_type = "C32";
            LS_INFO << "lidar type (override): C32" << LS_END;
        } else {
            LS_ERROR << "Unknown lidar_type_override: " << type
                     << " (expected one of c16_3, C16, c32_3, C32); using auto-detection"
                     << LS_END;
            return determineLidarType();
        }

        if (msop_return_byte_ == 0x39) return_mode = 2;
