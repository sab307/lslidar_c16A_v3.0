...
// special defines for lslidarlidar support
    ...
    public:
        ...
        // Apply a lidar model configuration named by the `lidar_type_override`
        // parameter instead of sniffing model bytes out of the MSOP tail.
        bool applyLidarTypeOverride();

        // Fetch a DIFOP packet, either from its own socket or, when the lidar
        // sends DIFOP to the MSOP port, from the queue filled by poll().
    ...
    public:
        int msop_udp_port{};
        std::string lidar_type_override_;
        bool difop_shared_port_{false};
        uint8_t msop_return_byte_{0};
        std::queue<lslidar_msgs::msg::LslidarPacket> difop_queue_;
        std::mutex difop_queue_mutex_;
        std::condition_variable difop_queue_cv_;
        int difop_udp_port{};
        int scan_num{};
